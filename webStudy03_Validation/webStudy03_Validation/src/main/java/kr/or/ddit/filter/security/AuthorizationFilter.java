...
/**
 *	이미 인증을 거치고 온 요청에 대해 현재 보호 자원에 대한 접근 허용 여부를 확인할 필터
 */
public class AuthorizationFilter implements Filter{
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		Map<String, String[]> securedResources = 
				(Map)application.getAttribute(AuthenticationFilter.RESOURCEMAPNAME);
		...
		if(pass) {
			...
			// 권한 없으면 BAD REQUEST 응답
			resp.sendError(statusCode, String.format("%s 자원에 대한 권한이 없음.",uri));
		}
